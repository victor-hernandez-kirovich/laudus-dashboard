...
export default function IndicadoresFinancierosPage() {
  ...
  // Calcular Ratios de Liquidez (Current Ratio y Prueba Ácida) para cada fecha
  const calculateLiquidityRatios = (): RatioData[] => {
    return allData.map(record => {
      ...
      const inventariosRecord = records.find((r: any) => r.accountNumber === "1109")
      const inventarios = inventariosRecord?.assets || 0
      
      // Calcular Current Ratio
      const ratio = pasivos > 0 ? activos / pasivos : 0
      
      // Calcular Prueba Ácida = (Activos Corrientes - Inventarios) / Pasivos Corrientes
  ...
  // Calcular Estructura Financiera para cada fecha
  const calculateEstructuraFinanciera = (): EstructuraFinancieraData[] => {
    return allData.map(record => {
      const records = record.data || []
      ...
      // Calcular Endeudamiento = (Pasivo Total / Activo Total) * 100
      const endeudamiento = activoTotal > 0 ? (pasivoTotal / activoTotal) * 100 : 0
      
      // Calcular Autonomía = (Patrimonio / Activo Total) * 100
  ...
  const currentRatios = calculateLiquidityRatios()
  const workingCapitalData = calculateWorkingCapital()
  const estructuraData = calculateEstructuraFinanciera()
  const latestRatio = currentRatios[0]
  const latestCapital = workingCapitalData[0]
  const latestEstructura = estructuraData[0]
  ...
  return (
    <div>
      ...
      <div className='p-4 sm:p-6 lg:p-8 space-y-6 sm:space-y-8'>
        ...
        {/* ESTRUCTURA FINANCIERA - Siempre visible si hay datos */}
        {allData.length > 0 && (
          <div className='border-t-4 border-gray-200 pt-8 mt-8'>
            <Card>
              <div className='flex items-center justify-between gap-4 mb-6'>
                <h2 className='text-3xl font-bold text-gray-900'>Estructura Financiera</h2>
              </div>
            </Card>

            {/* Valores actuales */}
            <div className='grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6'>
              <Card title="Endeudamiento">
                <div className='text-center py-6'>
                  <div className='text-5xl font-bold text-red-600'>
                    {latestEstructura?.endeudamiento.toFixed(2)}%
                  </div>
              ...
              <Card title="Autonomía Financiera">
                <div className='text-center py-6'>
                  <div className='text-5xl font-bold text-green-600'>
                    {latestEstructura?.autonomia.toFixed(2)}%
                  </div>
                  <div className='mt-3 text-sm text-gray-600'>
            ...
            <EstructuraFinancieraChart data={estructuraData.slice(0, period)} />
          </div>
        )}
      </div>
    </div>
