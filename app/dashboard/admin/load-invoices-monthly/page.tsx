...
import { Download, Calendar, Receipt, CheckCircle, XCircle, Clock, Loader2 } from 'lucide-react'
...
const STORAGE_KEY = 'laudus-load-invoices-monthly-state'
...
export default function LoadInvoicesMonthlyPage() {
  ...
  const [dataStatus, setDataStatus] = useState<{ exists: boolean; records: number; lastUpdated: string | null } | null>(null)
  ...
  const checkDataStatus = useCallback(async () => {
    try {
      const response = await fetch(`/api/admin/load-invoices-monthly?period=${period}`)
      if (response.ok) {
  ...
  const handleLoadData = async () => {
    ...
    addLog(`📅 Iniciando carga de facturas mensuales`)
    addLog(`📊 Periodo: ${MONTHS[selectedMonth - 1].label} ${selectedYear}`)
    addLog('🔐 Conectando con Laudus API...')

    try {
      const response = await fetch('/api/admin/load-invoices-monthly', {
        method: 'POST',
  ...
  return (
    <div>
      <Header
        title='Cargar Facturas Mensuales'
        subtitle='Importar datos de facturas agrupadas por mes desde Laudus'
      />
                  ...
                  {dataStatus.exists ? (
                    <>
                      ...
                      <span className='text-green-800 font-medium'>
                        Datos existentes: {dataStatus.records} registros
                      </span>
                    </>
                  ) : (
                    <>
        ...
        <Card>
          <div className='text-sm text-gray-600 space-y-2'>
            ...
            <ul className='list-disc list-inside space-y-1'>
              <li>Este proceso extrae las facturas agrupadas por mes desde Laudus</li>
              <li>Los datos incluyen: total, devoluciones, neto, margen, descuentos, cantidad</li>
