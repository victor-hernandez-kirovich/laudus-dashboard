...
export default function LoadDataPage() {
  ...
  const handleLoadData = async () => {
    ...
    if (enabledEndpoints.length === 0) {
      addLog('âŒ Error: Debes seleccionar al menos un endpoint')
      return
    }
    ...
    if (targetDate > today) {
      addLog('âŒ Error: No se pueden cargar datos de fechas futuras')
      return
    }
    ...
    addLog(`ðŸ“… Iniciando carga de datos para ${selectedDate}`)
    addLog(`ðŸ“Š Endpoints seleccionados: ${enabledEndpoints.map(e => e.label).join(', ')}`)
    addLog('ðŸ” Autenticando con Laudus API...')
    ...
    try {
      ...
      if (result.details) {
        ...
        addLog(` Fecha: ${result.details.date}`)
        addLog(` Endpoints: ${result.details.endpoints.join(', ')}`)
        addLog('')
        ...
        // Mark all enabled endpoints as loading (workflow in progress)
        setEndpoints(prev => prev.map(endpoint => 
          endpoint.enabled ? { ...endpoint, status: 'loading' as const } : endpoint
        ))
      }
      // Local execution support (legacy)
      ...
      addLog(`âŒ Error fatal: ${error instanceof Error ? error.message : 'Error desconocido'}`)
      setEndpoints(prev => 
  ...
  const getStatusText = (endpoint: EndpointStatus) => {
    switch (endpoint.status) {
      ...
      case 'loading':
        return 'Cargando...'
      case 'success':
        return `âœ“ ${endpoint.records} registros`
      case 'error':
        return `âœ— ${endpoint.error || 'Error'}`
    }
  }
  ...
  return (
    <div>
      <Header
        ...
        subtitle='Importar datos histÃ³ricos desde la API de Laudus ERP'
      />

      <div className='p-4 sm:p-6 lg:p-8 space-y-6'>
        {/* ConfiguraciÃ³n */}
        <Card title='ConfiguraciÃ³n de Carga'>
        ...
        <Card>
          <div className='flex items-start gap-3 text-sm text-gray-600'>
            ...
            <div className='space-y-2'>
              <p>
                <strong>Nota:</strong> Esta herramienta permite cargar datos histÃ³ricos desde la API de Laudus ERP.
              </p>
              <ul className='list-disc list-inside space-y-1 ml-2'>
                <li>Los datos se guardan en la misma estructura que la automatizaciÃ³n diaria</li>
                <li>Si ya existen datos para la fecha seleccionada, serÃ¡n sobrescritos</li>
                <li>El proceso puede tardar varios minutos dependiendo del tamaÃ±o de los datos</li>
