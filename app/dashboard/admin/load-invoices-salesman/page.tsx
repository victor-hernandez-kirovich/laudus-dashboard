...
import { Download, Calendar, Users, CheckCircle, Clock, Loader2 } from 'lucide-react'
...
export default function LoadInvoicesSalesmanPage() {
  ...
  const checkDataStatus = useCallback(async () => {
    try {
      const response = await fetch(`/api/admin/load-invoices-salesman?period=${period}`)
      if (response.ok) {
  ...
  const startPolling = useCallback((jobId: string) => {
    ...
    pollingRef.current = setInterval(async () => {
      ...
      try {
        const dataResponse = await fetch(`/api/admin/load-invoices-salesman?period=${period}`)
        if (dataResponse.ok) {
          const dataResult = await dataResponse.json()
          
          if (dataResult.dataExists && dataResult.records > 0) {
            addLog(`✅ Datos detectados: ${dataResult.records} vendedores`)
  ...
  const handleLoadData = async () => {
    ...
    addLog(`📅 Iniciando carga de facturas por vendedor`)
    addLog(`📊 Periodo: ${MONTHS[selectedMonth - 1].label} ${selectedYear}`)
    addLog('🔐 Conectando con Laudus API...')

    try {
      const response = await fetch('/api/admin/load-invoices-salesman', {
        method: 'POST',
  ...
  return (
    <div>
      <Header
        title='Cargar Facturas por Vendedor'
        subtitle='Importar datos de facturas agrupadas por vendedor desde Laudus'
      />
                  ...
                  {dataStatus.exists ? (
                    <>
                      ...
                      <span className='text-green-800 font-medium'>
                        Datos existentes: {dataStatus.records} vendedores
                      </span>
                    </>
                  ) : (
                    <>
        ...
        <Card>
          <div className='text-sm text-gray-600 space-y-2'>
            ...
            <ul className='list-disc list-inside space-y-1'>
              <li>Este proceso extrae las facturas agrupadas por vendedor desde Laudus</li>
              <li>Los datos incluyen: ID vendedor, nombre, neto, comisiones, margen, número de documentos, ticket promedio</li>
