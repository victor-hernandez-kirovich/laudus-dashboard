...
export async function POST(request: NextRequest) {
  try {
    ...
    const period = `${year}-${month.toString().padStart(2, '0')}`
    const jobId = `invoices-salesman_${period}_${Date.now()}_${Math.random().toString(36).substring(7)}`
    ...
    const jobStatus: LoadInvoicesJobStatus = {
      ...
      logs: [
        `[${new Date().toLocaleTimeString('es-CL')}] 📅 Iniciando carga de facturas por vendedor para ${period}`,
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (period) {
      const dataCollection = db.collection('invoices_by_salesman')
    ...
    const jobs = await collection
      .find({ jobId: { $regex: /^invoices-salesman/ } })
      .sort({ startedAt: -1 })
      .limit(10)
      .toArray()
    ...
    console.error('Error in load-invoices-salesman GET:', error)
    return NextResponse.json(
