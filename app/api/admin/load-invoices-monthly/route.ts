...
export async function POST(request: NextRequest) {
  try {
    ...
    const period = `${year}-${month.toString().padStart(2, '0')}`
    const jobId = `invoices-monthly_${period}_${Date.now()}_${Math.random().toString(36).substring(7)}`
    ...
    const jobStatus: LoadInvoicesJobStatus = {
      ...
      logs: [
        `[${new Date().toLocaleTimeString('es-CL')}] 📅 Iniciando carga de facturas mensuales para ${period}`,
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (period) {
      const dataCollection = db.collection('invoices_by_month')
    ...
    console.error('Error in load-invoices-monthly GET:', error)
    return NextResponse.json(
