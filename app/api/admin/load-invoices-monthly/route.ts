...
async function executeLocalPythonAsync(year: number, month: number, jobId: string): Promise<NextResponse> {
  try {
    ...
    const scriptPath = path.join(apiPath, 'scripts', 'fetch_invoices_monthly.py')
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Generar jobId único
    const jobId = `invoices_monthly_${year}_${month}_${Date.now()}_${Math.random().toString(36).substring(7)}`
    ...
    const jobStatus: LoadDataJobStatus = {
      ...
      endpoints: ['invoices_monthly'],
      status: 'pending',
      mode: 'github-actions',
      startedAt: new Date().toISOString(),
      results: [{
        endpoint: 'invoices_monthly',
        status: 'pending'
      }],
      logs: [
        `[${new Date().toLocaleTimeString('es-CL')}] 📅 Iniciando carga de facturas mensuales para ${year}-${month.toString().padStart(2, '0')}`,
      ]
    }
    ...
    const workflowResponse = await fetch(
      'https://api.github.com/repos/victor-hernandez-kirovich/laudus-api/actions/workflows/laudus-invoices-monthly.yml/dispatches',
      {
        method: 'POST',
        headers: {
    ...
    jobStatus.workflowName = 'Laudus Invoices Monthly'
    jobStatus.logs.push(
    ...
    console.error('Error in load-invoices-monthly API:', error)
    return NextResponse.json(
      {
        success: false,
